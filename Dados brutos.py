...
#armazenar no cache caso o data frame não seja filtrado. Assim, não será necessário fazer a conversão novamente dessa base de dados.
#Dessa forma, o arquivo já ficará armazenado na memória, caso a pessoa clique mais de uma vez no botão. Ao não precisar fazer a conversão todas as vezes,
#estamos otimizando o aplicativo.

@st.cache_data

#Função para poder fazer download da base dados
def converte_csv(df):
    return df.to_csv(index= False).encode('utf-8')
#para que o índice do dataframe não seja armazenado no arquivo.
#Por fim, acrescentamos .encode() e, entre aspas, a codificação utf-8 para que os textos que estejam com cedilha ou acentos não fiquem desconfigurados.
...
with st.spinner('Atualizando visualizações...'):
    time.sleep(1)
    with st.expander('Colunas'):                  #nomes das colunas q vão ser usadas /o que vem por padrão
...
with coluna1:
    ...
    nome_arquivo += '.csv'
with coluna2:
    st.download_button("Baixar", data= converte_csv(dados_filtrados), file_name = nome_arquivo,
